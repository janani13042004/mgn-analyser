...
# Enhanced Metagenome Pipeline GUI (All Steps Mandatory)
...
class MetagenomePipelineGUI(QDialog):
    ...
    def initUI(self):
        ...
        self.min_contig_label = QLabel('Min Contig Length:')
        self.min_contig_input = QLineEdit('1000')  # Default value
        self.min_contig_input.setFixedWidth(80)
        self.min_contig_input.setToolTip("Set the minimum length of contigs to keep in the assembly (default: 1000)")

        threads_layout.addWidget(self.threads_label)
        threads_layout.addWidget(self.threads_input)
        threads_layout.addWidget(self.min_contig_label)
        threads_layout.addWidget(self.min_contig_input)
    ...
    def run_full_pipeline(self):
        def pipeline():
            ...
            try:
                ...
                try:
                    min_contig_len = int(self.min_contig_input.text())
                    if min_contig_len < 100:
                        raise ValueError("Minimum contig length too small.")
                except:
                    min_contig_len = 1000
                    self.signal.log.emit("Invalid min contig length. Defaulting to 1000.")
                ...
                for i, (fwd, rev) in enumerate(zip(forward_reads, reverse_reads)):
                    ...
                    cmd_megahit = ['conda', 'run', '-n', self.megahit_env, 'megahit', '-1', trimmed_fwd,
                                   '-2', trimmed_rev, '-o', assembly_dir, '--min-contig-len', str(min_contig_len),
                                   '-t', str(threads)]
