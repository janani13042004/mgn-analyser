...
class TrimmomaticWorker(QThread):
    ...
    def __init__(self, forward_files, reverse_files, output_folder, adapter_file, customized_params, selected_database, blast_db_path):
        super().__init__()
    ...
    def run(self):
        ...
        try:
            ...
            if process.returncode != 0:
                ...
                self.finished.emit(False)
                return
            self.update_progress.emit("Assembly using bwa is completed")
            ...
            blast_cmd2 = f"conda run -n blast_env blastx -query {self.output_folder}/trinity_assembly.Trinity.fasta -db {self.blast_db_path} -out {self.output_folder}/blast.csv -outfmt \"6 qseqid sseqid pident length mismatch gapopen qstart qend sstart send evalue bitscore sseq\" -evalue 1e-5 -max_target_seqs 5 -num_threads 16"
...
class Trimmomatic_GUI(QDialog):
    ...
    def initUI(self):
        ...
        blast_db_layout = QHBoxLayout()
        blast_db_layout.addWidget(QLabel('Select BLAST Database:'))

        self.blast_db_text = QTextEdit()
        self.blast_db_text.setReadOnly(True)
        self.blast_db_text.setFixedHeight(50)  # Set fixed height to keep the text area small
        blast_db_layout.addWidget(self.blast_db_text)
        self.blast_db_label = QLabel(self)

        blast_db_layout.addWidget(self.blast_db_label)

        blast_db_select_button = QPushButton('Select', self)
        blast_db_select_button.clicked.connect(self.select_blast_db)
        blast_db_layout.addWidget(blast_db_select_button)
    ...
    def select_blast_db(self):
        db_path, _ = QFileDialog.getOpenFileName(self, 'Select BLAST Database', '', 'Database Files (*.fasta)')
        if db_path:
            self.blast_db_path = db_path
            self.blast_db_label.setText(db_path)

            # Add this line to update the QLabel with the BLAST database path:
            self.blast_db_label.setText("Selected BLAST Database: " + self.blast_db_path)
    ...
    def run_trimmomatic(self):
        ...
        self.worker = TrimmomaticWorker(
            self.forward_files, self.reverse_files, self.output_folder, self.adapter_file, customized_params, self.busco_database, self.blast_db_path
        )
