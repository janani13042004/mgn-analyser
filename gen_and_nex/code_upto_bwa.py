...
class GenomicsPipelineWorker(QThread):
    ...
    def run(self):
        ...
        for dir in [fastqc_dir, trimmed_dir, assembly_dir, sam_dir, freebayes_dir]:
            if not os.path.exists(dir):
                os.mkdir(dir)

        # Inside the GenomicsPipelineWorker class, in the run method:

        # After FastQC is completed
        ...
        # After Trimmomatic is completed
        ...
        # After BWA mapping is completed
        ...
        for forward_file, reverse_file in zip(self.trimmed_forward_files, self.trimmed_reverse_files):
            output_sam_path = os.path.join(sam_dir, f"{os.path.basename(forward_file).split('_')[0]}.sam")
        ...
        # After SAMtools rmdup is completed
        self.update_progress.emit("SAMtools rmdup is running.")
        for sam_file in self.sam_files:
            output_bam_file = os.path.join(sam_dir, os.path.basename(sam_file).replace('.sam', '_dedup.bam'))
        ...
        self.update_progress.emit("SAMtools rmdup is completed.")  # Updated message

        # After Freebayes variant calling is completed
        ...
        freebayes_cmd = f"freebayes -f {self.reference_genome} {' '.join(self.bam_files)} -v {self.freebayes_output}"
