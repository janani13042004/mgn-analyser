    ...
    def run_analysis(self):
        ...
        def run_star():
            try:
                ...
                cmd_1 = f'conda run -n star_env STAR --runMode genomeGenerate --genomeDir {os.path.join(self.output_folder, "genome_index")} --genomeFastaFiles {self.reference_file} --runThreadN 16'
                ...
                for forward_file, reverse_file in zip(self.forward_files, self.reverse_files):
                    ...
                    cmd_2 = f'conda run -n star_env STAR --genomeDir {os.path.join(self.output_folder, "genome_index")} --readFilesIn {forward_file} {reverse_file} --outFileNamePrefix {output_sam_path} --runThreadN 16 --readFilesCommand zcat'
        ...
        threading.Thread(target=run_star, daemon=True).start()
