...
class GenomicsPipelineWorker(QThread):
    ...
    def run(self):
        ...
        # SNP Finding in the background (placeholder)
        # Here you can add code for SNP finding using the generated VCF file or perform any additional analysis.
...
class GenomicsPipelineGUI(QDialog):
    ...
    def initUI(self):
        ...
        adapter_button = QPushButton('Select', self)
        ...
        output_button = QPushButton('Select', self)
        output_button.clicked.connect(self.select_output_folder)
        output_layout.addWidget(output_button)
        scroll_layout.addLayout(output_layout)

        # Customized parameters checkbox
        self.customized_params_checkbox = QCheckBox("Enable Customized Parameters")
        scroll_layout.addWidget(self.customized_params_checkbox)

        # Reference genome selection
        ref_genome_layout = QHBoxLayout()
        ref_genome_layout.addWidget(QLabel('Select Reference Genome:'))

        self.ref_genome_path = QLineEdit()
        self.ref_genome_path.setReadOnly(True)
        ref_genome_layout.addWidget(self.ref_genome_path)

        ref_genome_button = QPushButton('Select', self)
        ref_genome_button.clicked.connect(self.select_reference_genome)
        ref_genome_layout.addWidget(ref_genome_button)
        scroll_layout.addLayout(ref_genome_layout)
        ...
        scroll_layout.addWidget(run_button)
    ...
    def select_forward_files(self):
        ...
        if files:
            ...
            self.forward_paths.setText('\n'.join(self.forward_files))

    def add_forward_files(self):
        files, _ = QFileDialog.getOpenFileNames(self, "Add Forward Files", "", "FASTQ Files (*.fastq *.fq *.fastq.gz *.fq.gz)")
        if files:
            self.forward_files.extend(files)
            self.forward_paths.setText('\n'.join(self.forward_files))

    def delete_forward_file(self):
        selected_file, ok = QInputDialog.getItem(self, "Delete Forward File", "Select a file to delete:", self.forward_files, 0, False)
        if ok and selected_file:
            self.forward_files.remove(selected_file)
            self.forward_paths.setText('\n'.join(self.forward_files))
    ...
    def select_reverse_files(self):
        ...
        if files:
            ...
            self.reverse_paths.setText('\n'.join(self.reverse_files))

    def add_reverse_files(self):
        files, _ = QFileDialog.getOpenFileNames(self, "Add Reverse Files", "", "FASTQ Files (*.fastq *.fq *.fastq.gz *.fq.gz)")
        if files:
            self.reverse_files.extend(files)
            self.reverse_paths.setText('\n'.join(self.reverse_files))

    def delete_reverse_file(self):
        selected_file, ok = QInputDialog.getItem(self, "Delete Reverse File", "Select a file to delete:", self.reverse_files, 0, False)
        if ok and selected_file:
            self.reverse_files.remove(selected_file)
            self.reverse_paths.setText('\n'.join(self.reverse_files))
    ...
    def select_reference_genome(self):
        ...
        if file:
            ...
            self.ref_genome_path.setText(self.reference_genome)

    def run_pipeline(self):
        if not self.forward_files or not self.reverse_files:
            QMessageBox.warning(self, "Input Error", "Please select forward and reverse files.")
            return
        ...
        if not self.adapter_file:
            QMessageBox.warning(self, "Adapter File Error", "Please select an adapter file.")
            return

        if not self.reference_genome:
            QMessageBox.warning(self, "Reference Genome Error", "Please select a reference genome.")
            return

        self.customized_params = self.customized_params_checkbox.isChecked()
    ...
    def pipeline_finished(self, success):
        if success:
            QMessageBox.information(self, "Pipeline Finished", "Genomics pipeline completed successfully.")
        else:
            QMessageBox.critical(self, "Pipeline Error", "An error occurred during the genomics pipeline. Please check the inputs and try again.")
