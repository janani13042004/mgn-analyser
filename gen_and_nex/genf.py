...
class GenomicsPipelineWorker(QThread):
    ...
    def run(self):
        ...
        # Running BWA mapping
        self.update_progress.emit("Running BWA alignment.")
        for forward_file, reverse_file in zip(self.trimmed_forward_files, self.trimmed_reverse_files):
            forward_base = os.path.basename(forward_file).split('_')[0]
            sam_file = os.path.join(self.assembly_dir, f"{forward_base}.sam")
            alignment_report = os.path.join(self.sam_dir, f"{forward_base}_alignment_report.txt")

            bwa_mem_cmd = f"bwa mem -t 16 {self.reference_genome} {forward_file} {reverse_file} > {sam_file} 2> {alignment_report}"
            print(f"Running BWA mem command: {bwa_mem_cmd}")
            try:
                process = subprocess.Popen(bwa_mem_cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                stdout, stderr = process.communicate()
                print(f"BWA stdout: {stdout.decode()}")
                print(f"BWA stderr: {stderr.decode()}")
                if process.returncode != 0:
                    raise Exception(stderr.decode())
                self.sam_files.append(sam_file)
            except Exception as e:
                self.error_msg = f"BWA mem alignment failed for {forward_file} and {reverse_file}: {str(e)}"
                self.finished.emit(False)
                return

        self.update_progress.emit("BWA Assembly is completed.")

        # Converting SAM to BAM
        self.update_progress.emit("Converting SAM to BAM.")
        for sam_file in self.sam_files:
            bam_file = os.path.join(self.sam_dir, os.path.basename(sam_file).replace('.sam', '.bam'))
            samtools_view_cmd = f"conda run -n samtools_env samtools view -b -S {sam_file} > {bam_file}"
            print(f"Running SAMtools view command: {samtools_view_cmd}")
            try:
                ...
                self.error_msg = f"Converting SAM to BAM failed for {sam_file}: {str(e)}"
                self.finished.emit(False)
                return

        self.update_progress.emit("SAM to BAM conversion is completed.")

        # Sorting BAM files
        self.update_progress.emit("Sorting BAM files.")
        sorted_bam_files = []
        for bam_file in self.bam_files:
            sorted_bam_file = os.path.join(self.sam_dir, os.path.basename(bam_file).replace('.bam', '_sorted.bam'))
            samtools_sort_cmd = f"conda run -n samtools_env samtools sort {bam_file} -o {sorted_bam_file}"
            print(f"Running SAMtools sort command: {samtools_sort_cmd}")
            try:
                process = subprocess.Popen(samtools_sort_cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                stdout, stderr = process.communicate()
                print(f"SAMtools sort stdout: {stdout.decode()}")
                print(f"SAMtools sort stderr: {stderr.decode()}")
                if process.returncode != 0:
                    raise Exception(stderr.decode())
                sorted_bam_files.append(sorted_bam_file)
            except Exception as e:
                self.error_msg = f"Sorting BAM file failed for {bam_file}: {str(e)}"
                self.finished.emit(False)
                return
        self.bam_files = sorted_bam_files
        self.update_progress.emit("BAM files sorting is completed.")
        
        # Removing duplicates
        self.update_progress.emit("Removing duplicates using SAMtools.")
        dedup_bam_files = []
        for bam_file in self.bam_files:
            dedup_bam_file = os.path.join(self.sam_dir, os.path.basename(bam_file).replace('_sorted.bam', '_dedup.bam'))
            samtools_rmdup_cmd = f"conda run -n samtools_env samtools rmdup -S {bam_file} {dedup_bam_file}"
            print(f"Running SAMtools rmdup command: {samtools_rmdup_cmd}")
            try:
                ...
                self.error_msg = f"Removing duplicates failed for {bam_file}: {str(e)}"
                self.finished.emit(False)
                return
        self.bam_files = dedup_bam_files
        self.update_progress.emit("Duplicate removal is completed.")

        # Indexing BAM files
        self.update_progress.emit("Indexing BAM files.")
        for bam_file in self.bam_files:
            samtools_index_cmd = f"conda run -n samtools_env samtools index {bam_file}"
            print(f"Running SAMtools index command: {samtools_index_cmd}")
            try:
                process = subprocess.Popen(samtools_index_cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                stdout, stderr = process.communicate()
                print(f"SAMtools index stdout: {stdout.decode()}")
                print(f"SAMtools index stderr: {stderr.decode()}")
                if process.returncode != 0:
                    raise Exception(stderr.decode())
            except Exception as e:
                self.error_msg = f"Indexing BAM file failed for {bam_file}: {str(e)}"
                self.finished.emit(False)
                return
        self.update_progress.emit("BAM files indexing is completed.")
        ...
        self.update_progress.emit("Running Freebayes variant calling.")
        self.freebayes_output = os.path.join(self.freebayes_dir, "freebayes_out.vcf")
        freebayes_cmd = f"conda run -n variant_analysis_env freebayes -f {self.reference_genome} {' '.join(self.bam_files)} > {self.freebayes_output}"
        print(f"Running Freebayes command: {freebayes_cmd}")
        try:
            ...
            if process.returncode != 0:
                raise Exception(stderr.decode())
            self.update_progress.emit("Freebayes variant calling is completed.")
        except Exception as e:
            self.error_msg = f"Freebayes command failed: {str(e)}"
            self.finished.emit(False)
            return
