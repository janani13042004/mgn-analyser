...
class PopupWindow(QDialog):
        ...
        def run_rnaspades():
            ...
            try:
                ...
                if returncode == 0:
                    ...
                    self.signal.error.emit("rnaSPAdes command failed. Please check your inputs and try again.")
            except Exception as e:
                self.signal.error.emit(f"An error occurred while running rnaSPAdes: {str(e)}")

        # Start the thread
        threading.Thread(target=run_rnaspades, daemon=True).start()
    ...
    def show_error_msg(self, error_message):
        ...
        QMessageBox.critical(self, "rnaSPAdes Error", error_message)
