    ...
    def run_analysis(self):
        ...
        def run_hisat2():
            try:
                ...
                cmd_1 = f'conda run -n hisat_env hisat2-build {self.reference_file} {os.path.join(self.output_folder, "refgen_index")}'
                ...
                for forward_file, reverse_file in zip(self.forward_files, self.reverse_files):
                    ...
                    cmd_2 = f'conda run -n hisat_env hisat2 -p 16 -x {os.path.join(self.output_folder, "refgen_index")} -1 {forward_file} -2 {reverse_file} -S {output_sam_path}'
        ...
        threading.Thread(target=run_hisat2, daemon=True).start()

    def cancel_analysis(self):
        # Terminate the running hisat2 process
        subprocess.Popen("pkill hisat2", shell=True)
